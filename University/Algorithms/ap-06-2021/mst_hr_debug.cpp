...
int main()
{
   scanf("%d",&t);
    ...
    while (t--)
    {
        ...
        long long pre = m - last;//min(m-1,((n-2)*(n-1))/2); // total number of edges from all nodes, except the last node
        long long val=s-n+2; //value of edges from the last node
        long long ans=last*val+pre; // current answer

        cout << n << " " << m << " " << s << " " << last << " " << pre << " " << val << " " << ans << " ";


        if (m > last || n==2){
            cerr << "OK\n";
            printf("N %lld\n",ans); //the best answer: all nodes has value 1 and last has val
        }
        else 
        {
            cerr << "WOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOW\n";
